...
# CALIBRATION: Sigma sweep (2026-01-13) found sigma_meas was underestimated by 2.5x
# Without this calibration, false reject rate was 30-56% (should be <1%)
# See: logs/calibration_results_2026-01-13.md
SIGMA_MEAS_CALIBRATION = 2.5
...
class GlobalSimulationRunner:
    ...
    def _rand_sigmas(self, nu: int, noise_factor: float) -> Tuple[List[float], List[float]]:
        # Apply calibration factor to prevent systematic under-estimation
        sigma_meas = [
            random.uniform(0.5, 2.0) * (noise_factor / 2.0) * SIGMA_MEAS_CALIBRATION
            for _ in range(nu)
        ]
    ...
    def run(self):
        ...
        print(f" [!] CALIBRATED: sigma_meas × {SIGMA_MEAS_CALIBRATION:.1f} (from sweep 2026-01-13)")
