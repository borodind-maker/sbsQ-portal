...
# Precomputed Chi-square quantiles for 1-alpha1 and 1-alpha2
CHI2_QUANTILES_NU_2_9: Dict[int, Tuple[float, float]] = {
    1: (PHI ** 2, PI ** 2), # 1D case fallback
...
class GlobalSimulationRunner:
    ...
    def _temp_schedule(self, d: float) -> float:
        val = self.t_max * math.exp(-self.temp_alpha * d)
    ...
    def run(self):
        ...
        print(f" [!] GATE: ND Chi-square (STRICT) + FALLBACK HEURISTIC")
        print(f" [!] MODE: {self.mode} | DOF: {self.dof} | SEED: {self.seed}")
        print(f" [!] 1D COVERAGE: P(|Z|<=phi)={P_PHI_1D:.6f} -> alpha1={ALPHA1:.6f} | P(|Z|<=pi)={P_PI_1D:.6f} -> alpha2={ALPHA2:.6f}")
        if strict_enabled:
            q1, q2 = quantiles
            print(f" [!] Chi-square QUANTILES (DOF={self.dof}): q1={q1:.4f} | q2={q2:.4f} | multipliers: q1*x{self.q1_mult:.3f}, q2*x{self.q2_mult:.3f}")
        else:
            print(" [!] Chi-square QUANTILES: unavailable for this DOF -> heuristic fallback only")
        ...
        for i in range(1, self.cycles + 1):
            ...
            if random.random() < self.spoof_prob:
                ...
                # Bias relative to measurement noise
                bias = random.choice([-1.0, 1.0]) * self.spoof_bias_mult * meas_std
            ...
            sigma_total_sq = [(sigma_meas[j]**2 + sigma_pred[j]**2) for j in range(self.dof)]
            ...
            if any((not math.isfinite(v) or v <= 0.0) for v in sigma_total_sq):
                ...
                if strict_enabled:
                    ...
                    # Fallback to sum of Z-scores heuristic/sqrt
                    if d <= PHI:
                        status = "ACCEPTED"
                        self.stats["ACCEPTED"] += 1
                    elif d <= PI:
            ...
            if i % 1000 == 0 or i == 1:
                spoof_mark = " (SPOOFED)" if is_spoofed else ""
                print(f"Cycle {i:5}: d2={d2:8.3f} | d={d:6.3f} | noise={noise_factor:4.2f} | temp={temp:4.2f} | Result: {status}{spoof_mark}")
    ...
    def print_summary(self, strict_enabled: bool):
        ...
        print(f"{'='*80}")
        gate_label = "STRICT Chi^2" if strict_enabled else "HEURISTIC"
        print(f"  GATE MODE:          {gate_label} | DOF={self.dof}")
